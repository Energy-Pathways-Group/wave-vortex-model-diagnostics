...
% addpath('../wave-vortex-model-tutorials/CreateEnergyFluxDiagnosticsFile')
...
function CreateEnergyFluxDiagnosticsFile(filename,options)
arguments
    filename 
    options.stride = 1
    options.timeIndices
...
if options.shouldMeasureAntialiasingFlux
    [wvt_lowres, ncfile] = WVTransform.waveVortexTransformFromFile(filename,iTime=Inf);
...
if ~isfield(options,"outpath")
    ...
    if ~isempty(fpath)
        outpath = fullfile(fpath,strcat(fname,"-diagnostics.nc"));
    else
        outpath = fullfile(strcat(fname,"-diagnostics.nc"));
    end
else
    outpath = options.outpath;
end
diagfile = NetCDFFile(outpath,shouldOverwriteExisting=true);
