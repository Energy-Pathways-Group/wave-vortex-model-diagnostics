...
col{"te_damp"} = [205 253 197]/255;
col{"sink"} = [245 194 193]/255;

if options.shouldSeparateClosureRegion
    forcing_fluxes_jk = self.quadraticEnergyFluxesTemporalAverage(energyReservoirs=cat(2,options.energyReservoirs,EnergyReservoir.total),timeIndices=options.timeIndices);
    % exact_forcing_fluxes = self.exactEnergyFluxesTemporalAverage(timeIndices=options.timeIndices);
    ...
    forcing_fluxes = self.filterFluxesForReservoir(forcing_fluxes_jk,filter=@(v) sum(sum(v(NoDamp))));
    ...
    for iForce=1:length(forcing_fluxes)
        forcing_fluxes(iForce).te_damp = forcing_fluxes_damp(iForce).te_gmda + forcing_fluxes_damp(iForce).te_wave;
        forcing_fluxes(iForce).te_quadratic = forcing_fluxes(iForce).te_gmda + forcing_fluxes(iForce).te_wave + forcing_fluxes(iForce).te_damp;
    end
    % forcing_fluxes_unfiltered = self.filterFluxesForReservoir(forcing_fluxes_jk,filter=@(v) sum(sum(v(:))));
    [reservoirEnergy, t] = self.quadraticEnergyOverTime(energyReservoirs=options.energyReservoirs,timeIndices=options.timeIndices);

    % should probably estimate this at least?
    reservoirEnergy(3).name = "te_damp";
    reservoirEnergy(3).fancyName = "closure region";
    reservoirEnergy(3).energy = zeros(size(reservoirEnergy(1).energy));
    ...
    options.energyReservoirs = cat(2,options.energyReservoirs,EnergyReservoir.damp);

    
else
    forcing_fluxes = self.quadraticEnergyFluxesSpatialTemporalAverage(energyReservoirs=cat(2,options.energyReservoirs,EnergyReservoir.total),timeIndices=options.timeIndices);
    exact_forcing_fluxes = self.exactEnergyFluxesSpatialTemporalAverage(timeIndices=options.timeIndices);
    [reservoirEnergy, t] = self.quadraticEnergyOverTime(energyReservoirs=options.energyReservoirs,timeIndices=options.timeIndices);
end
...
for iFlux=1:length(forcing_fluxes)
    ...
    box = [];
    if forcing_fluxes(iFlux).te_quadratic/self.flux_scale/2 > options.fluxTolerance
    ...
    elseif forcing_fluxes(iFlux).te_quadratic/self.flux_scale/2 < -options.fluxTolerance
