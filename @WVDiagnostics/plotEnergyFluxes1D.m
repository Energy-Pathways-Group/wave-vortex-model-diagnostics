...
%
% - Topic: Figures (flux diagnostics)
% - Declaration: fig = plotEnergyFluxes1D(self,options)
% - Parameter options.timeIndices: (optional) Indices of model times to average/plot (default: Inf -> all times)
% - Parameter options.visible: (optional) Figure visibility (default: "on")
% - Parameter options.triadLineWidth: (optional) Line width for triad/inertial flux lines (default: 2)
% - Parameter options.forcingLineWidth: (optional) Line width for forcing flux lines (default: 1.5)
% - Parameter options.fluxTolerance: (optional) Minimum absolute flux magnitude to display (default: 5e-2)
% - Parameter options.keFractions: (optional) Fractional ticks for KE/(KE+APE) axis (default: [.01,0.025,.1,.25,.5,.75,.9,0.975,.99])
% - Parameter options.forcingFluxAttributes: (optional) Array/struct to override forcing attributes (color, fancyName, alpha)
