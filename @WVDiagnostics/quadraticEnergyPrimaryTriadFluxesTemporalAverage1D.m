...
for idx=1:length(fluxes_g)
    ...
    fluxes_g(idx).te_gmda = flux_interp(val);
end
...
for idx=1:length(fluxes_w)
    ...
    fluxes_w(idx).te_wave = flux_interp(val);
end

inertial_fluxes_g(1) = fluxes_g([fluxes_g.name] == "gmda_gmda");
inertial_fluxes_g(1).name = "ggg";
inertial_fluxes_g(1).fancyName = "[g{\nabla}g]_g";

inertial_fluxes_g(2) = fluxes_g([fluxes_g.name] == "gmda_wave");
inertial_fluxes_g(2).te_gmda = inertial_fluxes_g(2).te_gmda + fluxes_g([fluxes_g.name] == "wave_gmda").te_gmda + M_ggw;
inertial_fluxes_g(2).name = "ggw";
inertial_fluxes_g(2).fancyName = "[g{\nabla}w]_g+[w{\nabla}g]_g+\mathcal{M} [g{\nabla}g]_w";

inertial_fluxes_g(3) = fluxes_g([fluxes_g.name] == "wave_wave");
inertial_fluxes_g(3).name = "wwg";
inertial_fluxes_g(3).fancyName = "[w{\nabla}w]_g";

inertial_fluxes_g(4).te_gmda = -M_ggw;
inertial_fluxes_g(4).name = "ggw";
inertial_fluxes_g(4).fancyName = "-\mathcal{M} [g{\nabla}g]_w";

inertial_fluxes_w(1) = fluxes_w([fluxes_w.name] == "wave_wave");
inertial_fluxes_w(1).name = "www";
inertial_fluxes_w(1).fancyName = "[w{\nabla}w]_w";

inertial_fluxes_w(2) = fluxes_w([fluxes_w.name] == "gmda_wave");
inertial_fluxes_w(2).te_wave = inertial_fluxes_w(2).te_wave + fluxes_w([fluxes_w.name] == "wave_gmda").te_wave + M_wwg;
inertial_fluxes_w(2).name = "wwg";
inertial_fluxes_w(2).fancyName = "[g{\nabla}w]_w+[w{\nabla}g]_w+\mathcal{M} [w{\nabla}w]_g";

inertial_fluxes_w(3).te_wave = -M_wwg;
inertial_fluxes_w(3).name = "wwg";
inertial_fluxes_w(3).fancyName = "-\mathcal{M} [w{\nabla}w]_g";

inertial_fluxes_w(4) = fluxes_w([fluxes_w.name] == "gmda_gmda");
inertial_fluxes_w(4).name = "ggw";
